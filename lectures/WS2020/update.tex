%!Tex root = CoronaABM.tex


%\begin{frame}{Social Network in Covid-19 ABM}
%
%General idea
%
%\end{frame}


%\begin{frame}[fragile]
%	\frametitle{Code}
%	
%
%
%\end{frame}

\section{Update Function and Run Function}

\begin{frame}[fragile]
	\frametitle{Update Function}
	\bi 
	\item<1-> Choose queuing order of agents (\textit{np.random.choice}).
	\item<2-> For each agent:
	\bi 
	\item<2-> Check and potentially update state \ra state-dependent action  
	\item<2-> Potentially infect others in network with certain probability
	\ei
	\ei
	\begin{overlayarea}{\linewidth}{100cm}
\begin{onlyenv}<1,2>
\begin{lstlisting}[basicstyle=\tiny\ttfamily, tabsize=6] 
def update(t_now):
	queue = np.random.choice(agents, size=N_AGENTS, replace=False)
	for ag in queue:
\end{lstlisting}
\end{onlyenv}

\begin{onlyenv}<3>
\begin{lstlisting}[basicstyle=\tiny\ttfamily, tabsize=6] 
def update(t_now):
	queue = np.random.choice(agents, size=N_AGENTS, replace=False)
	for ag in queue:
		if ag.state == "susceptible":
			pass  # Do nothing
\end{lstlisting}
\end{onlyenv}

\begin{onlyenv}<4>
\begin{lstlisting}[basicstyle=\tiny\ttfamily, tabsize=6] 
def update(t_now):
	agent_list = np.random.choice(agents, size=N_AGENTS, replace=False)
	for ag in queue:
		if ag.state == "susceptible":
			pass  # Do nothing
		if ag.state == "exposed":
			# Potentially become infectious
			if t_now >= ag.infectious_period[0]:
				if ag.symptomatic:
					ag.state = "inf_presympt"  # pre-symptomatic
				else:
					ag.state = "inf_asympt"	# asymptomatic
\end{lstlisting}
Switch from latent to infectious \textit{state} (pre-symptom or asymptomatic) when \textit{infectious\_period} starts
\end{onlyenv}
\begin{onlyenv}<5>
\begin{lstlisting}[basicstyle=\tiny\ttfamily, tabsize=6] 
def update(t_now):
	queue = np.random.choice(agents, size=N_AGENTS, replace=False)
	for ag in queue:
		if ag.state == "susceptible":
			...
		if ag.state == "exposed":
			...
		if ag.state == "inf_presympt":  
			if t_now >= ag.t_onset_symptoms:
				ag.state = "inf_sympt"
\end{lstlisting}
Switch from pre-symptomatic to symptomatic when incubation period is over.
\end{onlyenv}

\begin{onlyenv}<6>
\begin{lstlisting}[basicstyle=\tiny\ttfamily, tabsize=6] 
def update(t_now):
	queue = np.random.choice(agents, size=N_AGENTS, replace=False)
	for ag in queue:
		if ag.state == "susceptible":
			...
		if ag.state == "exposed":
			...
		if ag.state == "inf_presympt":  
			...
		if ag.state[0:3] == "inf":  # "inf_asympt", "inf_presympt", "inf_sympt"
			if t_now >= ag.infectious_period[1]:
				if ag.fatal_case:
					ag.state = "dead"
				else:
					ag.state = "recovered" 
\end{lstlisting}
If agent in \textit{state ="inf\_..."} and the infectious period is over, then either recover or die.
\end{onlyenv}

\begin{onlyenv}<7>
\begin{lstlisting}[basicstyle=\tiny\ttfamily, tabsize=6] 
I_SYMPT, I_PRESYMPT, I_ASYMPT = 1, 0.5, 0.2

def update(t_now):
	queue = np.random.choice(agents, size=N_AGENTS, replace=False)
	for ag in queue:
		...
		if ag.state[0:3] == "inf":
			# Get Infectiousness:
			if ag.state == "inf_sympt":
				p_i = ag.base_infectiousness * I_SYMPT
			elif ag.state == "inf_presympt":
				p_i = ag.base_infectiousness * I_PRESYMPT
			elif ag.state == "inf_asympt":
				p_i = ag.base_infectiousness * I_ASYMPT
				
			# Loop through contacts and expose them
			linked_contacts = list(Net.adj[ag.id])  # Indices of neighbours
			for c in linked_contacts:
				contact_person = agents[c]		
				if np.random.random() < p_i:
					if contact_person.state == "susceptible":
						catch_virus(contact_person, t_now)
\end{lstlisting}
%If infectious, get current infection prob.\ and expose all neighbours 
\end{onlyenv}
\end{overlayarea}
\end{frame}


